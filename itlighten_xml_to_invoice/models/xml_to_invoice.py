...
class XmlImportWizard(models.TransientModel):
    ...
    def validations(self, bill):
        ...
        #    ref = filename
        if invoice_type == 'out_invoice':
            invoice_exists = self.validate_duplicate_invoice(rfc_receptor, filename, uuid)
        else:
            invoice_exists = self.validate_duplicate_invoice(rfc_emisor, filename, uuid)
        if invoice_exists:
    ...
    def validate_duplicate_invoice(self, vat, filename, uuid):
        """
        ...
        #if self.invoice_type == 'out_invoice' or self.invoice_type == 'out_refund':
            # FACTURA CLIENTE
        domain = ['|',('ref', 'ilike', uuid),('ref', 'ilike', filename)]
